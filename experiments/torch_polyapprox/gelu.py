...
from torch.distributions import StudentT as nct
...
def gelu_prime(x: Tensor) -> Tensor:
    ...
    return Normal(0,1).cdf(x) + x * Normal(0,1).log_prob(x).exp()


def gelu_dbl_prime(x: Tensor) -> Tensor:
    """Second derivative of GELU(x)"""
    return (2 - x**2) * Normal(0, 1).log_prob(x).exp()

def gelu_ev(mu: Tensor, sigma: Tensor) -> Tensor:
    """Expected value of GELU(x) under N(mu, sigma)"""
    denom = (1 + sigma**2).sqrt()
    return mu * Normal(0, 1).cdf(mu / denom) + (sigma**2 / denom) * Normal(0, 1).log_prob(mu / denom).exp()

#def gelu_ev(mu, sigma):
#    """Expected value of GELU(x) under N(mu, sigma)"""
#    denom = torch.sqrt(1 + sigma ** 2)
...
def gelu_prime_ev(mu: Tensor, sigma: Tensor) -> Tensor:
    ...
    return Normal(0, 1).cdf(mu / denom) + Normal(0, 1).log_prob(mu / denom).exp() * inner

def gelu_dbl_prime_ev(mu: Tensor, sigma: Tensor) -> Tensor:
    """Expected value of GELU''(x) under N(mu, sigma)"""
    denom = (1 + sigma**2).sqrt()
    inner1 = 1 / denom - 3 * mu**2 / denom**3
    inner2 = -3 * mu / denom**3 + 9 * mu**3 / denom**5
    return Normal(0, 1).cdf(mu / denom) + Normal(0, 1).log_prob(mu / denom).exp() * (inner1 + inner2)
...
def integral_noncentral_t(n: int, a: float, b: float) -> float:
    """
    ...
    term1 = torch.exp(torch.lgamma((n + 1) / 2)) * 2 ** ((n - 1) / 2) / (2 * math.pi).sqrt()
    prob_term = nct(f, torch.tensor(delta)).cdf(a * (n + 1).sqrt())
...
def integral_noncentral_t_whole_line(n: int, a: float, b: float) -> float:
    """
